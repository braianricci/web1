...
let main = document.getElementById("lol");
...
window.onscroll = () => {
  if (window.scrollY >= stickyDistance) {
    // Si la distancia que hemos scrolleado es mayor a stickyDistance, hacer el navbar sticky
...
function ShowSlides() {
  ...
  if (slideIndex >= slides.length) {
    // Si llegamos al ultimo (o nos pasamos) volvemos al indice 0
    slideIndex = 0;
  }
...
const alphabet =
  "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"; //alfabeto, de estos caracteres se va a componer el captcha
...
function Generate() {
  ...
  else captchaResult.innerHTML = ""; //en caso de venir de un refresh, limpiamos la notificacion
}
...
function Compare(event) {
  ...
  if (captchaInput == captchaText.innerHTML) {
    ...
    captchaResult.innerHTML =
      "Captcha mal ingresado, por favor intente nuevamente";
...
async function AddSingle(event) {
  ...
  try {
    let response = await fetch(
      `https://648caa018620b8bae7ed3718.mockapi.io/productos`,
      {
        method: "POST",
    ...
    if (response.status == 201) console.log("Creado!");
...
async function DeleteLast(event) {
  ...
  try {
    if (idCount > 0) {
      ...
      let response = await fetch(
        `https://648caa018620b8bae7ed3718.mockapi.io/productos/${lastId}`,
        {
          method: "DELETE",
        }
      );

      if (response.ok) console.log("Borrado!");
    }
  } catch (error) {
    console.log(error);
  }
      ...
      if (response.ok) {
        ...
        for (let item of jsonCart) {
          response = await fetch(
            `https://648caa018620b8bae7ed3718.mockapi.io/productos/${item.id}`,
            {
              method: "DELETE",
            }
          );
        ...
        if (response.ok) console.log("Reseteado!");
      }
    }
  } catch (error) {
    console.log(error);
...
function ProcessCart(jsonCart) {
  ...
  if (jsonCart.length > 0) lastId = jsonCart[jsonCart.length - 1].id;
  else lastId = 0;
